...
execute store result score initernal.mob.y_pos RD.mobs.temp run data get entity @s Pos[1]
execute store result score RD.initernal.mob.threshold RD.mobs.temp run random value 1..3

scoreboard players operation initernal.mob.y_pos RD.mobs.temp += RD.initernal.mob.threshold RD.mobs.temp

execute store result storage rockietools:mob_data temp_warp.Pos int 1 run scoreboard players get initernal.mob.y_pos RD.mobs.temp
