...
scoreboard players set #const.20 RD.ai_timer 20

scoreboard players operation @s RD.ai_timer_2 %= #const.20 RD.ai_timer

execute if score @s RD.ai_timer matches 1 at @s run playsound minecraft:entity.wither.spawn hostile @a ~ ~ ~ 1 2

execute if score @s RD.ai_timer_2 matches 1 at @s run function rd_custom_ai:custom_mobs/nuka_creeper/count
