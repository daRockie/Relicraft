...
# 鉱石のオフセット
execute store result entity @s[predicate=rd_custom_ai:percent/20] data.ore[{type:"ruby"}].offset int 1 run random value -48..13

execute store result entity @s[predicate=rd_custom_ai:percent/40] data.ore[{type:"peridot"}].offset int 1 run random value -20..30
